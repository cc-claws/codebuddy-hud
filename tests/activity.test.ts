...
describe('renderActivityLine', () => {
  describe('tools segment', () => {
    ...
    it('shows top 4 tools by frequency', () => {
      const tools = [
      ...
      assert.ok(line !== null);
      const plain = stripAnsi(line!);
      // All 4 tools should appear (each has count 1, top 4 shown)
      assert.ok(plain.includes('Read'));
    ...
    it('counts completed tools by name', () => {
      const tools = [
    ...
    it('error tools counted as completed', () => {
      const tools = [
        makeTool('Read', 'completed', 3),
      ...
      assert.ok(line !== null);
      const plain = stripAnsi(line!);
      // Running tool shows with ◐
      assert.ok(plain.includes('Write'));
      // Error tool counted as completed
      assert.ok(plain.includes('✓ Read'));
      assert.ok(plain.includes('✓ Bash'));
    });
  });
